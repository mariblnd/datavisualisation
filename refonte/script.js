...
function handleWaterQualityData(response) {
    if (response.data && Array.isArray(response.data)) {
        ...
        if (meanLi) {
            meanLi.textContent = ` ${meanNumericValue.toFixed(2)}`;
        }
        ...
        meanExploitationsElement.textContent = `${meanExploitations.toFixed(2)}`;
...
function createBarChart(departments, nitrateAverages, exploitationData) {
    ...
    let type = 'bar';
    let yText = 'Taux de Nitrate en mg/L';
    let titleChart = 'Moyenne des taux de nitrate selon le département';
    ...
    function updateChart(sortedData, includeExploitation) {
        const datasets = [{
            label: 'Moyenne des Taux de Nitrate (mg/L)',
        ...
        if (includeExploitation) {
            type = 'bubble';
            titleChart = 'Moyenne des taux de nitrate et nombre d\'exploitations agricoles en fonction des départements';
            yText = '';
            ...
            titleChart = 'Moyenne des taux de nitrate en fonction des départements';
            yText = 'Taux de nitrate en mg/L';
            type = 'bar';
            datasets.push({
                type: 'bar',
            });
        ...
        waterQualityChart.options.scales.y.title.text = yText;
        waterQualityChart.options.plugins.title.text = titleChart; 
    ...
    const exploitationChart = new Chart(ctxExploitation, {
        ...
        options: {
            ...
            plugins: {
                ...
                title: {
                    display: true,
                    text: "Nombre d'exploitations agricoles",
                    color: 'white',
                    font: {
                        size: 20,
                    },
...
const departments = ['Paris', 'Alpes-Maritime', 'Seine-et-Marne', 'Orne', 'Seine-Saint-Denis', 'Indre', 'Ardèche', 'Pyrénées-Atlantique', 'Ardennes', 'Vaucluse', 'Vienne', 'Aude', 'Haute-Marne', 'Aube', 'Manche', 'Essonne', 'Dordogne', 'Lot-et-Garonne', 'Aisne', 'Charente-Maritime', 'Indre-et-Loire', "Côtes-d'Armor", "Eure", "Calvados", "Marne", "Mayotte", "Charente", "Loire-Atlantique", "Morbihan", "Yvelines", "Pyrénées-Orientales", "Hérault"];
const exploitationData = [8, 1992, 2364, 4714, 16, 3738, 3748, 9784, 2623, 4860, 4044, 6103, 1850, 4885, 8038, 665, 6319, 5801, 4472, 5807, 3619, 7316, 3696, 5267, 13231, 4312, 4880, 4885, 5673, 807, 3227, 7891];
    ...
    const camembertChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                title: {
                    display: true,
                    text: "Nombre de sources conformes à la loi",
                    color: 'white',
                    font: {
                        size: 20,
                    },
